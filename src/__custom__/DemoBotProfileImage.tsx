...
function DemoBotProfileImage({ size, profileUrl }: { size: number; profileUrl?: string }) {
  ...
  const _profileUrl = profileUrl ?? botInfo?.profileUrl ?? botUser?.profileUrl;

  if (_profileUrl) {
    if (isMaybeFavicon(_profileUrl)) {
      return (
        <FaviconContainer size={size}>
          <FaviconImage size={size} src={_profileUrl} alt={'bot profile'} />
        </FaviconContainer>
      );
    }

    return <img src={_profileUrl} style={{ borderRadius: '50%', width: size, height: size }} alt={'bot profile'} />;
  }
...
export default DemoBotProfileImage;
