...
import BotMessageBottom from '../components/BotMessageBottom';
import SourceContainer, { Source } from '../components/SourceContainer';
...
const MultipleTokenTypeContainer = styled.div<{ customBackgroundColor?: string }>`
  border-radius: 16px;
  overflow: auto;

  &,
  & > div {
    background-color: ${({ theme, customBackgroundColor }) => customBackgroundColor ?? theme.bgColor.incomingMessage};
  }
`;
...
const RegexText = ({ children, patterns }: { children: string; patterns: RegexTextPattern[] }) => {
  ...
  return <DemoTextContainer>{convertedNodes}</DemoTextContainer>;
};

export default function DemoTokensBody({ tokens, sources, backgroundColor }: TokensBodyProps) {
  const { enableSourceMessage } = useConstantState();

  return (
    <MultipleTokenTypeContainer className="sendbird-word" customBackgroundColor={backgroundColor}>
