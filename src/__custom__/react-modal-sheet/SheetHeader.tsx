...
const SheetHeader = React.forwardRef<any, SheetDraggableProps>(({ children, style, disableDrag, ...rest }, ref) => {
  ...
  const indicator1Transform = useTransform(indicatorRotation, (r: any) => `translateX(2px) rotate(${r}deg)`);

  const indicator2Transform = useTransform(indicatorRotation, (r: any) => `translateX(-2px) rotate(${-1 * r}deg)`);
