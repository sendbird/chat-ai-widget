...
const Sheet = React.forwardRef<any, SheetProps>(
  (
    ...
    const zIndex = useTransform(y, (value: any) => (value >= windowHeight ? -1 : 9999999));

    const visibility = useTransform(y, (value: any) => (value >= windowHeight ? 'hidden' : 'visible'));
    ...
    if (snapPoints) {
      ...
      snapPoints = snapPoints.map((point: any) => {
    ...
    const onDragEnd = useEvent((_, { velocity }: PanInfo) => {
      if (velocity.y > DRAG_VELOCITY_THRESHOLD) {
        ...
        if (snapPoints) {
          const snapToValues = snapPoints.map((p: any) => sheetHeight - Math.min(p, sheetHeight));
