...
const PriceText = styled(UILabel)`
  font-weight: bold;
`;

const OriginalPriceText = styled(UILabel)`
  padding-left: 4px;
...
const RecommendItemsMessage = ({ data }: { data: FunctionCallResponse }) => {
  const items = data?.recommend_items as unknown as Item[];
        ...
        {items.map((item) => (
          <CarouselItem key={item.name}>
            ...
            >
              ...
              <div>
                <PriceText type={'subtitle1'} color={'onbackground1'}>
                  ${item.price}
                </PriceText>
                {item.original && (
                  <OriginalPriceText type={'subtitle2'} color={'onbackground2'}>
                    {item.original}
                  </OriginalPriceText>
                )}
              </div>
