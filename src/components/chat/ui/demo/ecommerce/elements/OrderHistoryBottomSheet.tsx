...
const OrderHistoryBottomSheet = ({
  ...
  const handleListRowClick = (id: string) => {
    sendUserMessage({ message: `Details for Order #${id}` });
  ...
  return (
    ...
    >
      <BottomSheetContainer>
        <BottomSheetHeader>
          <UILabel type={'h2'}>Order history</UILabel>
          <CloseIcon
            onClick={() => {
              setBottomSheetOpen(false);
            }}
            ...
            historyList.slice(0, 4).map((history) => {
              return (
                ...
                >
                  <ListRow
                    key={history.id}
                    icon={<ItemImageComponent image={history.items[0].image} width={44} height={44} />}
                    title={
                      <div
                        style={{
                          display: 'flex',
